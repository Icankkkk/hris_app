...
class PayrollController extends Controller implements HasMiddleware
{
    ...
    public static function middleware()
    {
        return [
            ...
            new Middleware(PermissionMiddleware::using(['payslip-view|payroll-list']), only: ['getMyPayslips', 'getMyPayslip']),
            new Middleware(PermissionMiddleware::using(['payslip-download|payroll-list']), only: ['downloadPayslip']),
        ];
    }
    ...
    public function getMyPayslips(Request $request)
    {
        ...
        try {
            $payslips = $this->payrollRepository->getMyPayslips(
                $validated['year'] ?? null,
                $validated['row_per_page'] ?? 12
            );

            return ResponseHelper::jsonResponse(true, 'My Payslips Retrieved Successfully', PaginateResource::make($payslips, PayrollDetailResource::class), 200);
        } catch (\Throwable $e) {
            return ResponseHelper::jsonResponse(false, 'Internal Server Error: ' . $e->getMessage(), null, 500);
        }
    }
    ...
    /**
     * Get specific payslip for the authenticated employee
     */
    public function getMyPayslip(string $id)
    {
        try {
            $payslip = $this->payrollRepository->getMyPayslip($id);

            return ResponseHelper::jsonResponse(true, 'Payslip Retrieved Successfully', new PayrollDetailResource($payslip), 200);
        } catch (ModelNotFoundException $e) {
    ...
    /**
     * Download PDF payslip for specific payroll detail
     */
    public function downloadPayslip(string $id)
    {
        try {
            $pdf = $this->payrollRepository->downloadPayslip($id);
