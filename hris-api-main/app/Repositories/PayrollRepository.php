...
class PayrollRepository implements PayrollRepositoryInterface
{
    ...
    public function getMyPayslips(?int $year, int $rowPerPage = 12)
    {
        $user = Auth::user();
        $employeeProfile = $user ? $user->employeeProfile : null;

        if (! $employeeProfile) {
            return new \Illuminate\Pagination\LengthAwarePaginator([], 0, $rowPerPage);
        }

        $query = PayrollDetail::with(['payroll', 'employee.user', 'employee.jobInformation.team'])
    ...
    public function downloadPayslip(string $id)
    {
        ...
        $user = Auth::user();
        if ($user && $user->hasRole('employee') && ! $user->hasAnyRole(['manager', 'hr', 'finance'])) {
            if ($detail->employee->user_id !== $user->id) {
                throw new \Exception('Unauthorized to download this payslip', 403);
            }
        }
