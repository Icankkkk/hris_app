...
const handleDownloadPayslip = async (detailId, empName) => {
  try {
    const blob = await payrollStore.downloadPayslip(detailId);
    const url = window.URL.createObjectURL(new Blob([blob], { type: "application/pdf" }));
    const link = document.createElement("a");
    link.href = url;
    const safeName = (empName || "Employee").replace(/\s+/g, "_");
    link.download = `Slip_Gaji_${safeName}_${payroll.value?.month || ""}_${payroll.value?.year || ""}.pdf`;
          ...
          <tbody>
            <tr v-for="emp in filteredEmployees" :key="emp.id"
              ...
              <td class="py-4 px-4 text-center">
                <button
                  v-if="emp.detail_id"
                  @click="handleDownloadPayslip(emp.detail_id, emp.name)"
                  title="Unduh Slip Gaji PDF"
                  class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-blue-50 text-[#0C51D9] hover:bg-[#0C51D9] hover:text-white rounded-[10px] text-xs font-semibold transition-all duration-200 shadow-sm"
                >
